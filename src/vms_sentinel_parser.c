...
#define ISINSTANCE(parser) dc_device_isinstance((parser), &vms_sentinel_parser_vtable)

#define SZ_HEADER 82

typedef struct vms_sentinel_parser_t vms_sentinel_parser_t;

struct vms_sentinel_parser_t {
	dc_parser_t base;
};

static dc_status_t vms_sentinel_parser_set_data (dc_parser_t *abstract, const unsigned char *data, unsigned int size);
...
static dc_status_t
vms_sentinel_parser_set_data (dc_parser_t *abstract, const unsigned char *data, unsigned int size)
{
	return DC_STATUS_SUCCESS;
}
...
static dc_status_t
vms_sentinel_parser_get_datetime (dc_parser_t *abstract, dc_datetime_t *datetime)
{
...
static dc_status_t
vms_sentinel_parser_get_field (dc_parser_t *abstract, dc_field_type_t type, unsigned int flags, void *value)
{
	if (abstract->size < SZ_HEADER)
		return DC_STATUS_DATAFORMAT;
...
static dc_status_t
vms_sentinel_parser_samples_foreach (dc_parser_t *abstract, dc_sample_callback_t callback, void *userdata)
{
	const unsigned char *data = abstract->data;
